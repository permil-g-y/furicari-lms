...
>
> 最終更新: **Phase 7 完成版**（main = origin/main = Vercel Production）
...
| 資料 | 役割 |
| --- | --- |
| **本書（`FURICARI_HANDOFF.md`）** | プロジェクト全体の現況。**いまの姿**と回帰テスト |
| `PHASE7_ADMIN_SPEC.md` | 管理画面の設計判断 |
| `FURICARI_LEGACY_CONTENT_INTEGRATION.md` | 既存 Notion 教材の**実物調査と設計判断**（未実装） |
| `FURICARI_LEGACY_INTEGRATION_PLAN.md` | 既存教材統合の**実装計画**（未実装） |

> 既存教材の統合は **計画段階**であり、まだ 1 行も実装されていない。
> 本書の内容は Phase 7 完成版の姿のままである。

---

## 0. Phase 7 完成版の現況（ここだけ読めば今の姿が分かる）

### 稼働環境

| | |
| --- | --- |
| 本番 | https://furicari-lms.vercel.app （Vercel Production / main に自動追随） |
| リポジトリ | github.com/permil-g-y/furicari-lms （**現在 public**） |
| DB / 認証 | Supabase（招待制。自己サインアップは無効） |
| 動画 | Cloudflare Stream（Require Signed URLs 有効） |
| メール | Resend 経由の Custom SMTP（送信元 `no-reply@dacho.info`） |

### 実装済みの機能

**受講生向け**
- ログイン / 初回パスワード設定 / パスワード再設定（すべて日本語メール）
- TOP（学習中のコース・学習サマリー・新着・お知らせ）
- コース一覧 / コース詳細（カリキュラム、未受講はロック表示）
- 動画一覧 / 検索 / お気に入り / 学習履歴 / お知らせ / マイページ
- 動画閲覧（Cloudflare 署名再生、再生位置の保存、90% 自動完了、手動完了）

**管理者向け（`/admin`）**
- ダッシュボード（受講生数・動画の状態・「⚠ 要対応」）
- 受講生一覧 / 詳細（メール・最終ログイン・最終学習・進捗）
- 受講生の招待（受講コースと期限を同時に設定）
- 受講権限の付与 / 解除 / 期限変更（無期限・3ヶ月・6ヶ月・1年・日付指定）
- レッスンの編集 / 並び替え / 公開切替 / 動画アップロード / 実尺同期
- お知らせの作成 / 編集 / 公開 / 予約公開
...
| 対象 | 件数 |
| --- | --- |
| コース / チャプター / レッスン | 6 / 25 / 90 |
| **実動画が入っているレッスン** | **1**（残り 89 本はダミープレイヤー） |
| 公開中のレッスン | 90 |
| お知らせ | 1 |
| ユーザー | 2（運営 1 = admin / テスト受講生 1） |

### DB（Supabase）

| テーブル | 用途 |
| --- | --- |
| `profiles` | 表示名・role・通知設定。role の昇格は列レベル権限 + トリガで防止 |
| `categories` / `tools` / `courses` / `chapters` / `lessons` | 教材 |
| `user_course_enrollments` | 受講権限（`expires_at` が NULL なら無期限） |
| `lesson_progress` / `lesson_view_events` / `lesson_favorites` / `course_favorites` | 学習進捗 |
| `announcements` / `announcement_reads` | お知らせと既読 |
| `admin_audit_logs` | 運営操作の記録（append-only） |

関数：`is_admin()`（SECURITY DEFINER）、`admin_list_students()`（同上・admin 以外は例外）

**RLS の考え方**
- 受講生は自分の行だけ。admin には別ポリシーを **追加**している（緩めていない）
- **受講生向けの取得は、公開条件をクエリでも明示する。**
  RLS は「admin に全件見せる」分岐も持つため、RLS だけに頼ると
  admin が受講生画面で下書き・予約公開を見てしまう

### 環境変数（名前のみ。値は Vercel / .env.local にのみ存在）

| 変数名 | 用途 | ブラウザへ露出 |
| --- | --- | --- |
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase 接続 | する（公開前提） |
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | 同上 | する（公開前提） |
| `NEXT_PUBLIC_SITE_URL` | メールの戻り先 | する |
| `SUPABASE_SECRET_KEY` | **招待のみ**。RLS を貫通する | **絶対にしない** |
| `CLOUDFLARE_STREAM_CUSTOMER_CODE` | 再生 URL | しない |
| `CLOUDFLARE_STREAM_SIGNING_KEY_ID` | 署名 | しない |
| `CLOUDFLARE_STREAM_SIGNING_KEY_PEM` | 署名 | しない |
| `CLOUDFLARE_ACCOUNT_ID` | 管理 API | しない |
| `CLOUDFLARE_STREAM_API_TOKEN` | 管理 API（Stream:Edit のみ） | しない |

### 外部サービスの設定

- **Supabase Auth**：自己サインアップ無効 / メール認証のみ / Site URL = 本番 URL /
  Redirect URLs にローカルも許可 / Custom SMTP（Resend）/ 招待・再設定メールは日本語
- **Cloudflare Stream**：Require Signed URLs 有効。署名は RS256、TTL 2 時間
- **Resend**：`dacho.info` を検証済み
...
今後の変更で壊してはいけないもの。**変更のたびにここを通すこと。**
...
| # | 確認 | 期待 |
| --- | --- | --- |
| A1 | 未ログインで各ページへ | すべて `/login` へ |
| A2 | ログイン直後の最初の遷移 | **エラー画面が出ない**（PGRST303 を吸収している） |
| A3 | パスワード再設定 | 日本語メールが届き、別ブラウザでも再設定できる |
| A4 | 招待 → 初回パスワード設定 → ログイン | 付与コースが見られる |

**B. 受講権限（最重要。admin では再現しない）**

| # | 確認 | 期待 |
| --- | --- | --- |
| B1 | 受講権限が一部だけの受講生でログイン | — |
| B2 | TOP / 動画一覧 / 検索 / お気に入り / 履歴 | **未受講コースへの `/watch/` リンクが 0** |
| B3 | 未受講コースの詳細 | ロック表示。リンクは張らない |
| B4 | 未受講レッスンの URL 直打ち | コース詳細へ戻され、**署名トークンが発行されない** |
| B5 | 期限切れのコース | 未受講と同じ扱い |
| B6 | ログイン済み非 admin で `/admin` | TOP へ戻される |

**C. 動画**

| # | 確認 | 期待 |
| --- | --- | --- |
| C1 | PC / Mobile 390 で実動画を再生 | iframe は **1 つだけ**・16:9 |
| C2 | 30 秒以上再生 → 離脱 → 戻る | 続きから再開する |
| C3 | 90% まで再生 | 自動で視聴済みになる |
| C4 | 管理画面から動画をアップロード | UID 自動紐付け → 実尺取得 → ready → 公開 |

**D. 管理画面**

| # | 確認 | 期待 |
| --- | --- | --- |
| D1 | 受講権限の付与 / 解除 / 期限変更 | 受講生側へ即反映。監査ログに残る |
| D2 | 期限に日付を指定 | **JST のその日いっぱい**まで有効 |
| D3 | レッスンの並び替え | 番号が飛ばない（01, 02, 03 …） |
| D4 | レッスンを下書きにする | 受講生からは消え、**管理画面には残る** |
| D5 | お知らせを未来の日時で公開 | 受講生側には出ない（**admin が見ても出ない**） |

**E. 表示・安全**

| # | 確認 | 期待 |
| --- | --- | --- |
| E1 | PC 1440 / Mobile 390 | 横スクロールなし |
| E2 | Console | エラー 0 |
| E3 | クライアントバンドル | 秘密情報 0 件 |
| E4 | `.env.local` | Git 未追跡 |

### 特に壊れやすい箇所

- **リンクを張る場所を増やしたら** → 必ず `LessonLink` を使う（判定を書き足さない）
- **受講生向けのデータ取得を足したら** → 公開条件をクエリで明示する（RLS 任せにしない）
- **日付を扱ったら** → JST で解釈する（Supabase も Vercel も UTC）
- **プレイヤーを触ったら** → PC と Mobile の両方を実寸で確認する（プレビュー枠では再現しない不具合がある）
